...
/**
 * Class implements parser for objects of class {@link Complex}.
...
public final class ComplexParser {
    ...
    /**
     * Constructor initializes new {@code ComplexParser} using string to be parsed as input.
     * @param string    string to be parsed.
     */
    public ComplexParser(String string) {
        this.string = string;
    }

    /**
     * Method that parses complex number from stored string.
     * @return  complex number of type {@link Complex} parsed from string
     *          input by constructor {code ComplexParser(String string)}.
     */
     ...
     * @throws ComplexParserException    if reading is attempted after the end of string is reached.
     */
    private String currentCharacter() {
        if (isEndOfString()) {
            throw new ComplexParserException("Trying to read after the end of string.");
        }
     ...
     * @throws ComplexParserException    if reading is attempted before the beginning of string
     *                                          (if current position in string is <= 0).
     */
    private String previousCharacter() {
        if (!hasPrevious()) {
            throw new ComplexParserException("Trying to read before the beginning of string.");
        }
    ...
    private Complex parse() {
        ...
        while (!isEndOfString()) {
            ...
            if (!isEndOfString() && currentCharacter().equals(IMAGINARY_UNIT)) {
                if (hasImaginary) {
                    throw new ComplexParserException("Two imaginary parts supplied.");
                } else {
                ...
                if (hasReal) {
                    throw new ComplexParserException("Two real parts supplied.");
                } else {
                    realPart = nextNumber;
                    hasReal = true;
                }
